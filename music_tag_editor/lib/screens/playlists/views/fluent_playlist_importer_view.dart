      ...
      content: Padding(
        ...
        child: Column(
          children: [
            Row(
              children: [
                Expanded(child: TextBox(controller: urlController, placeholder: 'URL da Playlist (Spotify ou YouTube)', suffix: IconButton(icon: const Icon(FluentIcons.search), onPressed: onScan))),
            ...
            if (isLoading)
              ...
              InfoBar(title: const Text('Erro'), content: Text(error!), severity: InfoBarSeverity.error)
              ...
              FilledButton(onPressed: onImportAll, child: Padding(padding: const EdgeInsets.all(12), child: Text('Importar ${tracks.length} Músicas'))),
            ] else
              Expanded(child: Center(child: Column(mainAxisSize: MainAxisSize.min, children: [Icon(FluentIcons.cloud_download, size: 64, color: FluentTheme.of(context).inactiveColor), const SizedBox(height: 16), const Text('Cole um link para começar.')]))),
          ],
        ),
      ),
    );
