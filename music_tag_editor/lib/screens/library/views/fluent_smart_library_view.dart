      ...
      content: TabView(
        ...
        tabs: [
          Tab(
            ...
            body: _SmartList(future: widget.topHitsFuture, onPlayTrack: widget.onPlayTrack, emptyText: 'Dê o play em algumas músicas primeiro!'),
          ),
          Tab(
            text: const Text('Descobertas Recentes'),
            icon: const Icon(FluentIcons.history),
            body: _SmartList(future: widget.recentDiscoveriesFuture, onPlayTrack: widget.onPlayTrack, emptyText: 'Suas novas músicas aparecerão aqui.'),
          ),
        ],
      ),
    );
...
class _SmartList extends StatelessWidget {
  ...
  const _SmartList({required this.future, required this.onPlayTrack, required this.emptyText});
  ...
  Widget build(BuildContext context) {
    return FutureBuilder<List<SearchResult>>(
      ...
      builder: (context, snapshot) {
        ...
        if (tracks.isEmpty) return Center(child: Text(emptyText, style: TextStyle(color: FluentTheme.of(context).inactiveColor)));
