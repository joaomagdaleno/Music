          ...
          children: [
            ...
            if (!_isHosting && !_isDiscovering) ...[
              fluent.Button(
                ...
                child: const Row(mainAxisAlignment: MainAxisAlignment.center, children: [Icon(fluent.FluentIcons.wifi), SizedBox(width: 8), Text('Hospedar Sessão')]),
              ),
              const SizedBox(height: 10),
              fluent.Button(
                onPressed: _startDiscovery,
                child: const Row(mainAxisAlignment: MainAxisAlignment.center, children: [Icon(fluent.FluentIcons.search), SizedBox(width: 8), Text('Entrar na Sessão')]),
              ),
              const SizedBox(height: 10),
              fluent.HyperlinkButton(
                onPressed: () {
                  Navigator.push(context, fluent.FluentPageRoute(builder: (context) => const PartyQueueScreen()));
                },
                child: const Row(mainAxisAlignment: MainAxisAlignment.center, children: [Icon(fluent.FluentIcons.q_r_code), SizedBox(width: 8), Text('Fila de Festa (QR)')]),
              ),
            ] else ...[
              if (_connectedGuests.isNotEmpty) ...[
                ...
                fluent.FilledButton(
                  onPressed: () {
                    Navigator.push(context, fluent.FluentPageRoute(builder: (context) => const RemoteLibraryScreen()));
                  },
              ...
              if (_isDiscovering && _foundDevices.isEmpty && _status != "Conectado!")
                const Text("\nNenhum dispositivo encontrado ainda...", style: TextStyle(fontSize: 12, color: fluent.Colors.grey), textAlign: TextAlign.center),
            ...
            if (!_isHosting && !_isDiscovering && _guestHistory.isNotEmpty) ...[
              ...
              const Padding(
                ...
                child: Text('Histórico de Convidados', style: TextStyle(fontWeight: FontWeight.bold)),
              ),
              SizedBox(
                height: 150,
                child: ListView.builder(
                  ...
                  itemBuilder: (context, index) {
                    ...
                    return fluent.ListTile(
                      ...
                      subtitle: Text('Última conexão: ${_formatDate(guest['last_connected'])}'),
                    );
                  },
                ),
              ),
          ...
          children: [
            ...
            if (!_isHosting && !_isDiscovering) ...[
              ...
              material.OutlinedButton.icon(
                ...
                onPressed: () {
                  Navigator.push(context, material.MaterialPageRoute(builder: (context) => const PartyQueueScreen()));
                },
              ),
            ] else ...[
              if (_connectedGuests.isNotEmpty) ...[
                const Icon(material.Icons.group, color: material.Colors.green, size: 48),
                const SizedBox(height: 10),
                Text("Broadcast Ativo (${_connectedGuests.length})", style: const TextStyle(fontWeight: FontWeight.bold)),
                const SizedBox(height: 10),
                ..._connectedGuests.entries.map((e) => material.ListTile(
                      leading: const Icon(material.Icons.person, size: 20),
                      title: Text(e.value),
                      trailing: const Icon(material.Icons.check, color: material.Colors.green),
                    )),
                const SizedBox(height: 20),
                material.ElevatedButton.icon(
                  ...
                  onPressed: () {
                    Navigator.push(context, material.MaterialPageRoute(builder: (context) => const RemoteLibraryScreen()));
                  },
                ),
              ] else ...[
              ...
              if (_isDiscovering && _foundDevices.isEmpty && _status != "Conectado!")
                const Text("\nNenhum dispositivo encontrado ainda...", style: TextStyle(fontSize: 12, color: material.Colors.grey)),
              if (_status != "Conectado!")
